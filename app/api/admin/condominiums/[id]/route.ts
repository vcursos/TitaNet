...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  return getOneHandler(prisma.condominium, params.id, opts)
}
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  const auth = await requireAuth(); if (auth.response) return auth.response
  return updateHandler(prisma.condominium, params.id, req, opts)
}
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  const auth = await requireAuth(); if (auth.response) return auth.response
  return deleteHandler(prisma.condominium, params.id)
}
